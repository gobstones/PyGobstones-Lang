...
class FileTestCase(TestCase):
    ...
    def __init__(self, parent, filename):
        ...
        self.assertTrue = self.parent.assertTrue
    ...
    def check_assert(self, results):
        for res, index in zip(results, range(len(results))):
            self.assertTrue(
                not res[1] in ["True", "False"] or res[1] == "True",
                "FileTestCase %s failed in the assertion number %s" % (
                    self.name(), index)
                )
    ...
    def run(self):
        if "board" in self.annotations.keys():
            if not self.annotations["board"].value is None:
                board = os.path.join(module_dir(self),
                                     self.annotations["board"].value)
            else:
                board = os.path.join(module_dir(self),
                                     self.filename[:-4] + ".gbb")
        else:
