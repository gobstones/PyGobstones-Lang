...
import common.utils
from lang.gbs_compiler import GbsCompileException
from lang.jsgbs.JSGbsCode import JSGbsCode

class JSGbsCompileException(lang.gbs_compiler.GbsCompileException):
...
"""
    ...
    def compile_routine_def(self, tree):
        ...
        code = JSGbsCode(tree, prfn, name, params, self.explicit_board)
        code.add_enter()
    ...
    def compile_proc_call(self, tree, code):
        ...
        compiled_args = [self.compile_expression(arg) for arg in args]
        code.push(tuple([procname, "t"]+compiled_args), near=tree)
    ...
    def compile_repeat(self, tree, code):
        ...
        code.push(('repeat', self.compile_expression(times), self.compile_high_order_block(body)), near=tree)

    def compile_high_order_block(self, tree):
        code = JSGbsCode(tree, "lambda", None, [])
        self.compile_block(tree, code)
        return repr(code)
    ...
    def binary_compiler(self, op):
        return lambda tree: self.compile_binary(op, tree)
    ...
    def compile_not(self, tree, code):
        ...
        return self.compile_unary("!", tree)

    def compile_or(self, tree, code):
        "Compile a short-circuiting disjunction."
        return self.compile_binary("||", tree)

    def compile_and(self, tree):
        "Compile a short-circuiting conjunction."
        return self.compile_binary("&&", tree)

    def compile_unary(self, op, tree):
        return "%s %s" % (op, self.compile_expression(tree.children[1]))

    def compile_binary(self, op, tree):
        return "%s %s %s" % (self.compile_expression(tree.children[2]), op, self.compile_expression(tree.children[3]))
...
def compile_program(tree):
    ...
    compiler = JSGbsCompiler()
    jsgbs_code = compiler.compile_program(tree)
    return jsgbs_code
